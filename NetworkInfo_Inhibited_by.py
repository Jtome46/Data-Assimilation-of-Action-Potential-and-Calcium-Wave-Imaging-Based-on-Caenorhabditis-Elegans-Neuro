...
import sys
sys.stdout = open('NetworkInfo_Inhibited_by.csv', 'wt')
...
with OWM('./.owm').connect() as conn:
    ...
    some_neuron_names = sorted(net.neuron_names())
    ...
    for neuron in some_neurons:
        #print((neuron.name(), ','.join(neuron.type())))
        ...
        for s in neuron.connection.get('either'):
            #print('   - Connection from %s -> %s (%s; %s)' % (
            #    s.pre_cell().name(), s.post_cell().name(), s.syntype(), s.synclass()))
        ...
        print(neuron.name() + ',' + str(len(conns["post"]["I"])) + ',' + ','.join(conns["post"]["I"]))
